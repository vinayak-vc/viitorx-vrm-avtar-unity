...
- **Feasibility spike IN PROGRESS — this is the exact resume point:** ONNX **downloaded + verified valid** (352 MiB, PyTorch-2.1 export) but **NOT yet imported into Sentis**. NEXT: put the ONNX under a regular `Assets/` folder (NOT StreamingAssets — Sentis imports `.onnx`→`ModelAsset`), refresh, and **read the import log for unsupported-operator warnings**. Clean → build `SentisPoseProvider` on Sentis. Rejected ops → **switch runtime to ONNX Runtime (DirectML)** (the one open risk; modern models often exceed Sentis op-support). The model file was only in the ephemeral session scratchpad — **re-download via the ADR-015 URL**.
- **Planned provider (once spike passes):** `SentisPoseProvider : IBodyTrackingProvider` (Tracking asmdef + `Unity.InferenceEngine` ref) mirroring `MediaPipePoseProvider`; camera → `TextureConverter.ToTensor` (384×288, normalize) → `Worker.Schedule` (GPUCompute) → readback → SimCC decode → 133→33 `JointId` (Unity hips-centred metres); wire behind `AppBootstrap.useSentis3dTracking` (default off) + MediaPipe fallback; add root/hip grounding (stop float) + velocity outlier-reject (leg jitter). Needs a live-validation/tuning loop like `poseFlipX`.
- **⚠️ Second Unity editor on this machine:** a project **`breakbuttler-base-project`** is also open and the MCP resolver routed port 6400 to it while ours was mid-reload. ALWAYS pin our instance explicitly (`set_active_instance viitorx-vrm-avtar-unity-base-project@70066c27`) and guard mutating `execute_code` on `Application.dataPath` containing `viitorx-vrm-avtar-unity`.
